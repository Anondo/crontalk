...
type translatorHelper struct {
	...
	translatedString *string  // the field which holds the translated string
}

func (t *translatorHelper) translateWeekMonth() (bool, error) {
	var v, v1, v2 int
	var mtext string
	...
	if t.ranged {
		*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+mtext)+mm[v1]+
			viper.GetString(configStr+"to")+mm[v2], t.index == 0) //if this is the first check
		*t.translatedString += helper.GetStrIfTrue(mm[v1]+viper.GetString(configStr+"to")+mm[v2],
			t.index > 0) //just keep adding the value & not the full sentence

	} else {
		*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+mtext)+mm[v], t.index == 0) //if this is the first check
		*t.translatedString += helper.GetStrIfTrue(mm[v], t.index > 0)                                   //just keep adding the value & not the full sentence
	}
	*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"and"), t.listed && t.index < t.cronListedLen-1) //print "and" for listed
	return true, nil
}

func (t *translatorHelper) translateDay() {
	...
	if t.ranged {
		*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+mtext)+viper.GetString(configStr+t.moment)+
			t.cronRange[0]+viper.GetString(configStr+"to")+t.cronRange[1], t.index == 0)
		*t.translatedString += helper.GetStrIfTrue(t.cronRange[0]+viper.GetString(configStr+"to")+t.cronRange[1],
			t.index > 0)
	} else {
		*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+mtext)+viper.GetString(configStr+t.moment)+
			t.cron, t.index == 0)
		*t.translatedString += helper.GetStrIfTrue(t.cron, t.index > 0)
	}
	*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"and"), t.listed && t.index < t.cronListedLen-1)
}

func (t *translatorHelper) translateMinuteAndHour() error {
	...
	for i, min := range mm { // nested loops are required as, if both the minute & hour values are listed ,the
		for j, hr := range hh { //time is be shown as, for each minute the listed hours
			...
			if !rangedM && !rangedH && !steppedHour && !steppedMinute { // if none of them are ranged or stepped then print like a normal time
				...
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"at")+pt, i == 0 && j == 0)
				*t.translatedString += helper.GetStrIfTrue(pt, i > 0 || j > 0)
			} else if (!rangedH && !steppedHour) && (rangedM || steppedMinute) { //or if only the minute is ranged or stepped
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"hour")+hr, true)
			} else if (rangedH || steppedHour) && (!rangedM && !steppedMinute) { //if only the hour is ranged or stepped
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"minute")+min, true)
			}

			*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"and"), (listedM || listedH) &&
...
func (t *translatorHelper) translateMinuteOrHour() {
	...
	if mVal == every { //if default
		...
		for i, hr := range hh { // iterating because could be a list
			if strings.Contains(hr, step) { // if the hour is stepped
				t.cron = hr
				t.moment = hour
				*t.translatedString += viper.GetString(configStr + "at_every_minute") // as the minute is * and minute will not be parsed later in this function
				t.translateStepValues()
				continue
			}
			hrr, ranged := helper.GetList(hr, rangee)
			if ranged { // checking if the value is ranged , different output if so
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"at_every_minute_of_hour")+hrr[0]+
					viper.GetString(configStr+"to")+hrr[1], i == 0)
				*t.translatedString += helper.GetStrIfTrue(hrr[0]+viper.GetString(configStr+"to")+hrr[1], i > 0)

			} else {
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"at_every_minute_of_hour")+hr, i == 0)
				*t.translatedString += helper.GetStrIfTrue(hr, i > 0)
			}

			*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"and"), listed && i < len(hh)-1)
		}
	} else {
		...
		for i, min := range mm {                 //iterating because could be a list
			...
			if ranged { //checking if the value is ranged
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"at")+viper.GetString(configStr+mStr)+
					" "+mr[0]+viper.GetString(configStr+"to")+mr[1], i == 0)
				*t.translatedString += helper.GetStrIfTrue(mr[0]+viper.GetString(configStr+"to")+mr[1], i > 0)
			} else {
				*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"at")+viper.GetString(configStr+mStr)+
					" "+min, i == 0)
				*t.translatedString += helper.GetStrIfTrue(min, i > 0)
			}

			*t.translatedString += helper.GetStrIfTrue(viper.GetString(configStr+"and"), listed && i < len(mm)-1)
		}
	}

}

func (t *translatorHelper) translateStepValues() error {
	...
	*t.translatedString += " " //adding a space for optical optimization
	...
	if ranged { //if ranged, instead of a default stopping range like normal(i.e 2/2) ones, the ranged value will be given
		if t.moment == week {
			...
			*t.translatedString += viper.GetString(configStr+"every") + stepValue + viper.GetString(configStr+"day_of_the_week") +
		...
		if t.moment == month {
			...
			i1, _ := strconv.Atoi(rValue[0])
			i2, _ := strconv.Atoi(rValue[1])
			*t.translatedString += viper.GetString(configStr+"every") + stepValue + viper.GetString(configStr+"month_of_the_year") +
				viper.GetString(configStr+"from") + months[i1] + viper.GetString(configStr+"to") + months[i2]
		}
		if t.moment == day {
			*t.translatedString += viper.GetString(configStr+"every") + stepValue + viper.GetString(configStr+"day_of_the_month") +
				viper.GetString(configStr+"from") + rValue[0] + viper.GetString(configStr+"to") + rValue[1]
		}
		if t.moment == hour {
			*t.translatedString += viper.GetString(configStr+"every") + stepValue + viper.GetString(configStr+"hour") +
				viper.GetString(configStr+"from") + rValue[0] + viper.GetString(configStr+"to") + rValue[1]
		}
		if t.moment == minute {
			*t.translatedString += viper.GetString(configStr+"every") + stepValue + viper.GetString(configStr+"minute") +
				viper.GetString(configStr+"from") + rValue[0] + viper.GetString(configStr+"to") + rValue[1]
		}
	} else { //if not ranged
